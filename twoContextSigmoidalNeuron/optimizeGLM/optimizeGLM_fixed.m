function [res,paramObj] = optimizeGLM_fixed(estimator,nLevels,sig,h,saveOut)
...
if strcmp(estimator,'mean')==1
    ...
    thetaH = sig;
    sigma = 1;
    xM = [ (xCore * sigma + thetaL); (xCore * sigma + thetaH) ];
elseif strcmp(estimator,'variance')==1
    thetaL = 1;
    thetaH = sig;
    mu = 0;
    xM = [ (xCore * thetaL + mu); (xCore * thetaH + mu) ];
else
...
folder = '/outputFolder/'; 
filename = ['discreteFixedGLM',estimator,num2str(nLevels),'levels',num2str(thetaH-thetaL),'sep',num2str(1000*h),'h.mat'];
